...
namespace ITScan
{
    public class MyDataSource : IDisposable
    {
        Identity _applicationId;
        IWindowsMessageHook _messageHook;

        public MyDataSource(Identity applicationId, Identity sourceId, IWindowsMessageHook messageHook)
        {
        ...
        public static MyDataSource GetDefault(Identity applicationId, IWindowsMessageHook messageHook)
        {
            ...
            if (result != TwainResult.Success)
            {
                var status = MyDataSourceManager.GetConditionCode(applicationId, null);
                throw new TwainException("Error getting information about the default source: " + result, result, status);
            }

            return new MyDataSource(applicationId, defaultSourceId, messageHook);
        }

        public static MyDataSource UserSelected(Identity applicationId, IWindowsMessageHook messageHook)
        {
            var defaultSourceId = new Identity();
        ...
        public static List<MyDataSource> GetAllSources(Identity applicationId, IWindowsMessageHook messageHook)
        {
            var sources = new List<MyDataSource>();
            ...
            {
                sources.Add(new MyDataSource(applicationId, id, messageHook));
            }
        ...
        public static MyDataSource GetSource(string sourceProductName, Identity applicationId, IWindowsMessageHook messageHook)
        {
